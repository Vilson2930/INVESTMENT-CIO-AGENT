...
def build_sp500_output():
    return {
        ...
        "metrics": {"operational_regime": "YELLOW_EXPENSIVE_BULL"},
        "risk": {
            "level": "HIGH",
            "score": None,
            "alerts": ["Valuation historicamente elevado."],
        },
        "data_quality": {"score": 78.0, "missing_fields": [], "warnings": []},
        "audit": {
...
def build_global_output():
    return {
        ...
        "risk": {
            "level": "CRITICO",
            "score": 100.0,
            "alerts": ["Survival Kill Switch ativo.", "Risco de ruína: ALTO"],
        },
        "data_quality": {"score": None, "missing_fields": [], "warnings": []},
        "audit": {
...
def test_build_synthesis():
    result = build_synthesis(build_sp500_output(), build_global_output())
    assert result["synthesis_version"] == "1.1"
...
def test_preserves_source_decisions():
    ...
    assert result["source_decisions"]["SP500_CYCLE_ATLAS"]["signal"] == "HOLD"
    assert result["source_decisions"]["SP500_CYCLE_ATLAS"]["new_contribution_equity"] == 0.60
    assert result["source_decisions"]["SP500_CYCLE_ATLAS"]["new_contribution_reserve"] == 0.40
    assert result["source_decisions"]["COPIAULTIMOROB"]["signal"] == "NEUTRO"
    assert result["source_decisions"]["COPIAULTIMOROB"]["final_verdict"] == "REPROVADO_OPERACIONALMENTE"
    assert result["source_decisions"]["COPIAULTIMOROB"]["committee_action"] == "BLOQUEAR_NOVAS_COMPRAS"
...
def test_preserves_audits():
    ...
    assert result["audit"]["sp500"]["engine_consistency_score"] == 92.0
    assert result["audit"]["sp500"]["data_quality_score"] == 78.0
    assert result["audit"]["copiaultimorob"]["ai_audit_score"] == 90.0
    assert result["audit"]["copiaultimorob"]["nvidia_audit_score"] == 90.0
...
def test_generic_interface():
    outputs = [build_global_output(), build_sp500_output()]
...
def test_missing_system():
    outputs = [build_sp500_output()]

    try:
        synthesize_outputs(outputs)
    except SynthesisError:
        print("OK - ausência de sistema detectada")
        return

    raise AssertionError(
        "A síntese deveria rejeitar a ausência do COPIAULTIMOROB."
    )
...
def run_all_tests():
    ...
    print("=" * 70)
    print("TODOS OS TESTES DO SYNTHESIS AGENT PASSARAM")
    print("=" * 70)
